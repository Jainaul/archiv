...
def modify_output_file(directory):
    ...
    # 从 "../inf.xlsx" 文件的 B1 单元格读取项目名称
    try:
        wb = load_workbook(excel_file)
        sheet = wb.active
        project_name = sheet["B1"].value
    ...
    # 从 "../inf.xlsx" 的 B2 单元格获取公司名称
    inf_file = "../inf.xlsx"
    try:
        inf_wb = load_workbook(inf_file)
        inf_sheet = inf_wb.active
        company_name = inf_sheet["B2"].value
    except FileNotFoundError:
        print(f"{inf_file} 不存在")
        return
    except KeyError:
        print("无法读取公司名称")
        return
    ...
    for root, dirs, files in os.walk(directory):
        for folder_name in dirs:
            ...
            # 在第13列写入公司名称，并设置为标题
            sheet.cell(row=1, column=13, value="公司名称")
            sheet.cell(row=2, column=13, value=company_name)

            # 在第14列，写入截断后的文件名
            sheet.cell(row=1, column=14, value="项目编号")
            for index in range(2, sheet.max_row + 1):
                file_number = sheet.cell(row=index, column=1).value
                truncated_number = file_number[:-16]
                sheet.cell(row=index, column=14).value = truncated_number
